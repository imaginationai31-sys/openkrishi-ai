    ...
    MaterialTheme {
        Surface(Modifier.fillMaxSize(), color = Color(0xFFF6FAF7)) {
            ...
            when (screen) {
                ...
                "advisory" -> if (advisoryResult == null && !analyzing) {
                    AdvisoryInputScreen(language, selectedCrop, { selectedCrop = it; selectedCropName = "" },
                        selectedCropName, { selectedCropName = it }, advisoryQuery, { advisoryQuery = it }, ::askAdvisory) { screen = "home" }
                } else {
                    ResultScreen(null, selectedCrop, selectedCropName, language, advisoryResult, advisoryError,
                        analyzing, { screen = "home" }, { askAdvisory(advisoryQuery) }, { screen = "voice" })
                }
